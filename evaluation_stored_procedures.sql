...
left join eval_status on course_assignments.teacher_num = eval_status.teacher_num and course_assignments.course_code = eval_status.course_code and eval_status.stu_num = (select stu_num from class_student_list where stu_num = stu) 
...
Create Procedure evaluate_teacher(IN stu int, IN t_num int, IN c_code int, in f_code int)
	Begin
		Insert into eval_status (stu_num, teacher_num, course_code, eval_date, form_code) values (stu, t_num, c_code, curdate(), f_code);
    End
DELIMITER ;
