...
use evaluation;
DELIMITER //
Create Procedure view_unevaluated_teacher(IN stu int)
	Begin
...
DELIMITER //
Create Procedure view_evaluated_teacher(IN stu int)
	Begin
...
Create Procedure open_student_info(IN stu int)
		BEGIN
			SELECT person.PERSON_ID,STU_NUM,FNAME,MNAME,LNAME,DOB, YR_START FROM EVALUATION.person JOIN EVALUATION.STUDENTS USING(person_id) where stu_num is not null and stu_num = stu and status_code = 1;
        END
DELIMITER ;
