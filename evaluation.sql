...
SELECT persons.person_id As "persons ID",teachers_code As "teachers Code",FNAME As "First Name",MNAME As "Middle Name",LNAME As "Last Name",DOB As "Date-of-Birth" FROM evaluation.persons LEFT JOIN evaluation.teachers ON persons.person_id = teachers.person_id where teachers_code is not null;
...
-- view all students
-- CREATE VIEW VIEW_TEACHERS AS SELECT TEACHERS_CODE FROM teacher_assignment where class_code = (select class_code from class_student_lists) ORDER BY TEACHERS_CODE;
