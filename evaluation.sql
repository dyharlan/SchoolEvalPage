...
=======
    ...
    FOREIGN KEY (teacher_num) REFERENCES teachers(teacher_num),
...
=======
SELECT course_assignments.teacher_num, COURSE_CODE, persons.FName, persons.Lname FROM course_assignments left join teachers on teachers.teacher_num = course_assignments.teacher_num left join persons on persons.person_id = teachers.person_id where class_code = (select class_code from class_student_list where stu_num = 202242069);
-- list the teachers the student has in their class with full info
SELECT course_assignments.class_code, teachers.teacher_num As "teachers Code", course_assignments.course_code As "Course Code", persons.FNAME As "First Name", persons.MNAME As "Middle Name", persons.LNAME As "Last Name", DOB As "Date-of-Birth" FROM evaluation.persons LEFT JOIN evaluation.teachers ON persons.person_id = teachers.person_id LEFT JOIN EVALUATION.course_assignments ON teachers.teacher_num = course_assignments.teacher_num where teachers.teacher_num is not null and teachers.teacher_num in (SELECT teacher_num FROM course_assignments 
where class_code = (select class_code from class_student_list where stu_num = 202213379)) AND course_assignments.class_code = (select class_code from class_student_list where stu_num = 202213379) order by persons.person_id asc;


-- list the teachers from an entire college campus
SELECT persons.person_id As "persons ID",teacher_num As "teachers Code",FNAME As "First Name",MNAME As "Middle Name",LNAME As "Last Name",DOB As "Date-of-Birth" FROM evaluation.persons LEFT JOIN evaluation.teachers ON persons.person_id = teachers.person_id where teacher_num is not null;
>>>>>>> Stashed changes
...
=======
select persons.fname, persons.lname, eval_status.teacher_num, eval_status.course_code, course_offerings.course_name FROM eval_status left join teachers on eval_status.teacher_num = teachers.teacher_num left join persons on teachers.person_id = persons.person_id left join course_offerings on eval_status.course_code = course_offerings.course_code where eval_status.stu_num = 202242069;
-- view the teachers with their name that the student has NOT evaluated
SELECT persons.fname, persons.lname, course_assignments.teacher_num, course_assignments.course_code, course_offerings.course_name FROM course_assignments 
...
left join teachers on course_assignments.teacher_num = teachers.teacher_num left join persons on teachers.person_id = persons.person_id
>>>>>>> Stashed changes
...
=======
SELECT persons.person_id As "persons ID",STU_NUM As "Student Number",FNAME As "First Name",MNAME As "Middle Name",LNAME As "Last Name",DOB As "Date-of-Birth", YR_START As "Year Started" FROM evaluation.persons LEFT JOIN evaluation.students ON persons.person_id = students.person_id where stu_num is not null;
-- view a specific student
SELECT persons.person_id As "persons ID",STU_NUM As "Student Number",FNAME As "First Name",MNAME As "Middle Name",LNAME As "Last Name",DOB As "Date-of-Birth", YR_START As "Year Started" FROM evaluation.persons LEFT JOIN evaluation.students ON persons.person_id = students.person_id where stu_num is not null and stu_num = 202269420;
